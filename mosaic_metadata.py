...
with open(os.path.join(pathway,"idmetadata.csv"),'wb') as csvfile:
        writer=csv.DictWriter(csvfile,fieldnames=["name","coordinate_system","id","first_acquired","last_acquired","quad_size","resolution","level"], delimiter=',')
        writer.writeheader()
def handle_page(page,year):
    ...
    for items in page['mosaics']:
        if items['name'].startswith('global_monthly_'+str(year)):
            ...
            pattern = '%Y-%m-%d'
            epoch_first = int(time.mktime(time.strptime(date_time, pattern)))*1000
            date_time = last_acquired.split("T")[0]
            pattern = '%Y-%m-%d'
            epoch_last = int(time.mktime(time.strptime(date_time, pattern)))*1000
            with open(os.path.join(pathway,"idmetadata.csv"),'a') as csvfile:
                writer=csv.writer(csvfile,delimiter=',',lineterminator='\n')
                writer.writerow([name,coordinate_system,ids,epoch_first,epoch_last,quad_size,resolution,level])
def idm(start,end):
